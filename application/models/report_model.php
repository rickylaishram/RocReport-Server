...
class Report_model extends CI_Model {
	...
	*/
	...
	function selectNearby($latitude, $longitude, $distance, $limit) {
		...
		$sql = "SELECT *, ( 6371000 * acos( cos( radians(?) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians(?) ) + sin( radians(?) ) * sin( radians( lat ) ) ) ) AS distance FROM ".$this->table['report']." HAVING distance < ? ORDER BY distance LIMIT 0 , ?";
