...
class Report_model extends CI_Model {
	...
	*/
	...
	function selectNearby($email, $latitude, $longitude, $distance, $offset, $limit, $orderby) {
		...
		$sql = "SELECT *, ( 6371000 * acos( cos( radians(?) ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians(?) ) + sin( radians(?) ) * sin( radians( latitude ) ) ) ) AS distance FROM ".$this->table['report']." HAVING distance < ? ORDER BY '.$orderby.' LIMIT ? , ?";
